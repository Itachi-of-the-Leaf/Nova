...
\subsection{We obtain Theorem 1 by showing that Uniform is in CoSBP.}
...
\item If x $\in$ $\Pi$Y es then Pr[M accepts] $\geq$  1+$\in$

\item If x $\in$ $\Pi$No then Pr[M accepts] $\leq$   1   ,
...
\item (Soundness:) For any function P $*$ and any x $\in$ $\Pi$No, Pr[V accepts] $\leq$ 1/3

\item (Completeness:) If x $\in$ $\Pi$Y es, Pr[V accepts] $\geq$ 2/3

\item (Zero Knowledge:) For any x $\in$ $\Pi$Y es, the distribution of S(x) is identical to the distribution of the transcript generated by $\langle$P, V $\rangle$ on input x.
...
\item Cn is a oracle circuit that maps n bits to n bits and makes oracle queries only to strings of length cn.

\item There exist families of sets \{An\}n$\geq$0, \{Bn\}n$\geq$0 $\subseteq$ \{0, 1\}cn such that for all n, CAn $\in$ $\Pi$Y and CBn $\in$ $\Pi$N
...
\item For every probabilistic polynomial-time Turing Machine M and infinitely many n, for every Di $\in$
...
\item Oracle Separation of NIPZK from SBP
...
\item ``AO asks $\langle$q, i$\rangle$'' is the event that ``the ith circuit query made by A is CO(q).'' For simplicity, we write this event as ``AO asks qi.''

\item ``AO gets $\langle$u, i$\rangle$'' is the event that ``CO(q) = u where q is the ith query''. Again, for simplicity, we write this event as ``AO gets ui.''

\item For S = $\langle$u1, . . . uk$\rangle$, ``AO gets S'' is the event that ``AO gets u1 and AO gets u2 and . . . AO gets uk (in that order)''.
...
\item Oracle Separation of PZK from CoSBP
...
\item Scott Aaronson. Impossibility of succinct quantum proofs for collision-freeness. Quantum Inf. Comput., 12(1- 2):21--28, 2012.

\item Scott Aaronson, Baris Aydinlioglu, Harry Buhrman, John M. Hitchcock, and Dieter van Melkebeek. A note on exponential circuit lower bounds from derandomizing arthur-merlin games. Electronic Colloquium on Computa- tional Complexity (ECCC), 17:174, 2010.

\item William Aiello and Johan Hastad. Statistical zero-knowledge languages can be recognized in two rounds. Journal of Computer and System Sciences, 42(3):327 -- 345, 1991.

\item Sanjeev Arora and Boaz Barak. Computational Complexity - A Modern Approach. Cambridge University Press, 2009.

\item Baris Aydinlioglu, Dan Gutfreund, John M. Hitchcock, and Akinori Kawachi. Derandomizing arthur-merlin games and approximate counting implies exponential-size lower bounds. Computational Complexity, 20(2):329-- 366, 2011.

\item Manuel Blum, Paul Feldman, and Silvio Micali. Non-interactive zero-knowledge and its applications (extended abstract). In Janos Simon, editor, Proceedings of the 20th Annual ACM Symposium on Theory of Computing, May 2-4, 1988, Chicago, Illinois, USA, pages 103--112. ACM, 1988.

\item Manuel Blum, Alfredo De Santis, Silvio Micali, and Giuseppe Persiano. Noninteractive zero-knowledge. SIAM
...
\item Elmar Bo¨hler, Christian Glaßer, and Daniel Meister. Error-bounded probabilistic computations between MA and AM. J. Comput. Syst. Sci., 72(6):1043--1076, 2006.

\item Ravi B. Boppana, Johan H$^\circ$astad, and Stathis Zachos. Does co-np have short interactive proofs? Inf. Process. Lett., 25(2):127--132, 1987.

\item Adam Bouland, Lijie Chen, Dhiraj Holden, Justin Thaler, and Prashant Nalini Vasudevan. On the power of statistical zero knowledge. In Chris Umans, editor, 58th IEEE Annual Symposium on Foundations of Computer Science, FOCS 2017, Berkeley, CA, USA, October 15-17, 2017, pages 708--719. IEEE Computer Society, 2017.

\item L. Fortnow. The complexity of perfect zero-knowledge. In Proceedings of the Nineteenth Annual ACM Symposium on Theory of Computing, STOC '87, page 204--209, New York, NY, USA, 1987. Association for Computing Machinery.

\item Rosario Gennaro, Daniele Micciancio, and Tal Rabin. An efficient non-interactive statistical zero-knowledge proof system for quasi-safe prime products. In Li Gong and Michael K. Reiter, editors, CCS '98, Proceedings of the 5th ACM Conference on Computer and Communications Security, San Francisco, CA, USA, November 3-5, 1998, pages 67--72. ACM, 1998.

\item Oded Goldreich and Shafi Goldwasser. On the limits of nonapproximability of lattice problems. J. Comput. Syst. Sci., 60(3):540--563, 2000.

\item Oded Goldreich, Silvio Micali, and Avi Wigderson. Proofs that yield nothing but their validity for all languages in NP have zero-knowledge proof systems. J. ACM, 38(3):691--729, 1991.

\item Oded Goldreich and Dana Ron. On testing expansion in bounded-degree graphs. Electronic Colloquium on Computational Complexity (ECCC), 7(20), 2000.

\item Oded Goldreich, Amit Sahai, and Salil P. Vadhan. Can statistical zero knowledge be made non-interactive? or on the relationship of SZK and NISZK. In Advances in Cryptology - CRYPTO '99, 19th Annual International Cryptology Conference, Santa Barbara, California, USA, August 15-19, 1999, Proceedings, volume 1666 of Lecture Notes in Computer Science, pages 467--484. Springer, 1999.

\item S Goldwasser, S Micali, and C Rackoff. The knowledge complexity of interactive proof-systems. In Proceedings of the Seventeenth Annual ACM Symposium on Theory of Computing, STOC '85, page 291--304, New York, NY, USA, 1985. Association for Computing Machinery.

\item Shafi Goldwasser, Silvio Micali, and Charles Rackoff.  The knowledge complexity of interactive proof systems.
...
\item Greg Kuperberg. How hard is it to approximate the jones polynomial? Theory Comput., 11:183--219, 2015.

\item Shachar Lovett and Jiapeng Zhang. On the impossibility of entropy reversal, and its application to zero-knowledge proofs. In Yael Kalai and Leonid Reyzin, editors, Theory of Cryptography - 15th International Conference, TCC 2017, Baltimore, MD, USA, November 12-15, 2017, Proceedings, Part I, volume 10677 of Lecture Notes in Computer Science, pages 31--55. Springer, 2017.

\item Lior Malka. How to achieve perfect simulation and a complete problem for non-interactive perfect zero-knowledge.
...
\item Tatsuaki Okamoto. On relationships between statistical zero-knowledge proofs. J. Comput. Syst. Sci., 60(1):47-- 108, 2000.

\item Chris Peikert and Vinod Vaikuntanathan. Noninteractive statistical zero-knowledge proofs for lattice problems. In David A. Wagner, editor, Advances in Cryptology - CRYPTO 2008, 28th Annual International Cryptology Con- ference, Santa Barbara, CA, USA, August 17-21, 2008. Proceedings, volume 5157 of Lecture Notes in Computer Science, pages 536--553. Springer, 2008.

\item Amit Sahai and Salil P. Vadhan. A complete problem for statistical zero knowledge. J. ACM, 50(2):196--249, 2003.

\item Thomas Watson. The complexity of estimating min-entropy. Computational Complexity, 25(1):153--175, 2016.
